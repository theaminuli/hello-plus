...
async function updateVersion() {
	...
	if (!fs.existsSync('./style.css')) {
		console.error('❌ Error: style.css file not found.');
		process.exit(1);
	}
	...
	const newStyleCssFile = styleCssFile.replace(
		/Version: (.*)/,
		`Version: ${newVersion}`
	);
	fs.writeFileSync('./style.css', newStyleCssFile);
	console.info(
		'✅  style.css file version updated',
		currentTag,
		'=>',
		newTag
	);
